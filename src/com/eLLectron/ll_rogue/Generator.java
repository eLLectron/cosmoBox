...
public class Generator {
	...
	Random fakeRand = new Random();
	Random rand = new Random(fakeRand.nextInt(99999));
	...
	public void gen(int[][] map, Player player) {
		do {
			for (int y = 0; y < 50; y++) {
				for (int x = 0; x < 50; x++) {
					if (noise(x, y, 3, rand.nextInt(999999999)) > 120) {
						map[y][x] = 1;
					}
				}
			}
	...
	public int noise(double x, double y, int nbOctave, int seed) {
		Random noiseRand = new Random(seed);
		int result = 0;
		...
		while (octave != 0) {
			...
			// int alt1 = (XYBASE_00 * (XYBASE_00 * XYBASE_00 * 15731 + 789221)
			// + 1376312589);
			// int alt2 = (XYBASE_10 * (XYBASE_10 * XYBASE_10 * 15731 + 789221)
			// + 1376312589);
			// int alt3 = (XYBASE_01 * (XYBASE_01 * XYBASE_01 * 15731 + 789221)
			// + 1376312589);
			// int alt4 = (XYBASE_11 * (XYBASE_11 * XYBASE_11 * 15731 + 789221)
			// + 1376312589);

			int alt1 = noiseRand.nextInt(999999999);
			int alt2 = noiseRand.nextInt(999999999);
			int alt3 = noiseRand.nextInt(999999999);
			int alt4 = noiseRand.nextInt(999999999);
			...
			/*
			 * START BLOCK FOR BICUBIC INTERPOLATION int bX2 = (bX * bX) >> 8;
			 * int bX3 = (bX2 * bX) >> 8; int _3bX2 = 3 * bX2; int _2bX3 = 2 *
			 * bX3; int alt12 = alt1 - (((_3bX2 - _2bX3) * (alt1 - alt2)) >> 8);
			 * int alt34 = alt3 - (((_3bX2 - _2bX3) * (alt3 - alt4)) >> 8);
			 * 
			 * int bY2 = (bY * bY) >> 8; int bY3 = (bY2 * bY) >> 8; int _3bY2 =
			 * 3 * bY2; int _2bY3 = 2 * bY3; int val = alt12 - (((_3bY2 - _2bY3)
			 * * (alt12 - alt34)) >> 8);
			 * 
			 * val *= 256; END BLOCK FOR BICUBIC INTERPOLATION
			 */
	...
	public int neighbors(int[][] map, int x, int y) {
		int n = 0;

		if (x == 0 || x == 49 || y == 0 || y == 49) {
			return n;
		}
		if (map[y - 1][x - 1] == map[x][y]) {
			n++;
		}
	...
	public void automata(int[][] map) {
		for (int y = 0; y < 50; y++) {
			for (int x = 0; x < 50; x++) {
				if (map[y][x] == 0) {
					if (neighbors(map, x, y) <= 3) {
						map[y][x] = 1;
					}
				}
				if (map[y][x] == 1) {
					if (neighbors(map, x, y) >= 6) {
						map[y][x] = 0;
					}
				}
			}
